...
class Student:
    def __init__(self,root):
        ...
        semester_combo = ttk.Combobox(current_course_frame,textvariable=self.var_semester ,font=("times new roman", 11, "normal"), state="readonly",
                                  width=20)
        semester_combo["values"] = ("Select a base", "Changhua", "Taichung")
        ...
        save_btn=Button(btn_frame,text="Save",command=self.add_data,font=("times new roman",11,"bold"),bd=0,bg="white",cursor='hand2' ,activebackground='white',
                        width=115,image=self.photobtn1,fg="white",compound="center")
        save_btn.place(x=30,y=3)

        # nút sửa thông tin sinh viên
        update_btn = Button(btn_frame, text="Edit",command=self.update_data, font=("times new roman", 11, "bold"), bd=0,bg="white",cursor='hand2' ,activebackground='white',
                        width=115,image=self.photobtn1,fg="white",compound="center")
        update_btn.place(x=180, y=3)

        # nút xóa thông tin sinh viên
        delete_btn = Button(btn_frame, text="Remove",command=self.delete_data, font=("times new roman", 11, "bold"),bd=0,bg="white",cursor='hand2' ,activebackground='white',
                        width=115,image=self.photobtn1,fg="white",compound="center")
        delete_btn.place(x=330, y=3)

        #nút làm mới
        reset_btn = Button(btn_frame, text="Refresh",command=self.reset_data, font=("times new roman", 11, "bold"), bd=0,bg="white",cursor='hand2' ,activebackground='white',
                        width=115,image=self.photobtn1,fg="white",compound="center")
        ...
        take_photo_btn = Button(btn_frame1, text="Get student photo",command=self.generate_dataset, font=("times new roman", 11, "bold"), bd=0,bg="white",cursor='hand2' ,activebackground='white',
        ...
        #nút tìm kiếm
        search_btn = Button(search_frame, text="Search", font=("times new roman", 11, "bold"),command=self.search_data,bd=0,bg="white",cursor='hand2' ,activebackground='white',
                        width=105,image=self.photobtn3,fg="white",compound="center")
        search_btn.grid(row=0, column=3,padx=4)

        #nút xem tất cả
        showAll_btn = Button(search_frame, text="Show all", font=("times new roman", 11, "bold"), command=self.fetch_data,bd=0,bg="white",cursor='hand2' ,activebackground='white',
                        width=105,image=self.photobtn3,fg="white",compound="center")
    ...
    def add_data(self):#Thêm dữ liệu
        ...
        if  self.var_std_name.get()=="" or self.var_std_id.get()=="" or self.var_div.get()=="":
            ...
            try:
                ...
                my_cursor=conn.cursor()
                dob_string = self.dob_entry.get_date().strftime('%Y-%m-%d')
                ...
                messagebox.showinfo("Success","Added student information successfully",parent=self.root)#thêm thành công
    ...
    def generate_dataset(self):#Chụp ảnh học sinh
        ...
        if  self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:#update thông tin học sinh
                ...
                #=========load haar===================
                face_classifier=cv2.CascadeClassifier("haarcascade_frontalface_default.xml")#模型檢測屏幕中的人臉
                ...
                while True:
                    ...
                    if face_cropped(frame) is not None:
                        img_id += 1
                        face = cv2.cvtColor(face_cropped(frame), cv2.COLOR_BGR2GRAY)
                        _, img_buffer = cv2.imencode('.jpg', face)
    ...
    def train_classifier(self):  # 训练人脸识别模型
        data_dir = "data"  # 训练一个人脸识别模型
        path = [os.path.join(data_dir, file) for file in os.listdir(data_dir)]

        faces = []
        ids = []
        for image in path:  # 目錄中的每個圖像
            img = PIL.Image.open(image).convert('L')  # convert ảnh về dạng pil
            imageNp = np.array(img, 'uint8')
            id = int(os.path.split(image)[1].split('.')[
                         1])  # Lấy ra id học sinh từ tên của ảnh( ví dụ: user.1.19.jpg -> id học sinh=1)
        ...
        messagebox.showinfo("Success", "Training dataset Completed", parent=self.root)  # thông báo train thành công
    ...
    def add_Classdata(self):#Thêm lớp học
            ...
            if self.var_class.get() == "" or self.var_nameclass.get() == "":
                ...
                messagebox.showerror("Error", "The class already exists! Please check again.", parent=self.root)
            else:
                try:
                    conn = mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)
                    my_cursor = conn.cursor()
                    ...
                    conn.close()
                    messagebox.showinfo("Success", "Added class information successfully",
                                        parent=self.root)
                except Exception as es:
                    messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
    ...
    def delete_Classdata(self):#Xóa lớp học
            if self.var_class== "" or self.var_nameclass.get() == "":
                messagebox.showerror("Error", "Please do not leave any field blank! ", parent=self.root)
            else:
                try:
                    ...
                    # self.fetch_data()p
                    conn.close()
                    messagebox.showinfo("Deletion", "Record deleted successfully", parent=self.root)
                    self.reset_Classdata()
                    self.fetch_Classdata()
                except Exception as es:
                    messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
    ...
    def search_Classdata(self):#Tìm thông tin lớp học
            if self.var_com_searchclass.get() == "" or self.var_searchclass.get() == "":
                messagebox.showerror("Error!", "Please provide complete information",parent=self.root)
                ...
                try:
                    ...
                    if (self.var_com_searchclass.get() == "Subject"):
                        self.var_com_searchclass.set("Subject")
                    elif (self.var_com_searchclass.get() == "Course"):
                        self.var_com_searchclass.set("Course")
                    ...
                    if (len(data) != 0):
                        ...
                        for i in data:
                            self.StudentTable.insert("", END, values=i)
                        messagebox.showinfo("Notification", "Yes" + str(len(data)) + " the record meets the condition!",parent=self.root)
                        conn.commit()
                    else:
                        self.StudentTable.delete(*self.StudentTable.get_children())
                        messagebox.showinfo("Notification", " There are no records that satisfy the condition!",parent=self.root)
                    conn.close()
                except Exception as es:
                    messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
