    ...
    def face_recog(self):#hàm nhận diện khuôn mặt
            ...
            def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):#xác định khuôn mặt trong khung hình
                ...
                for(x,y,w,h) in features:#với mỗi khuôn mặt đã nhận diện đc( x ,y,w,h tạo thành hình vuông khi nhận diện)
                    ...
                    if confidence>0:#Nếu tỉ lệ nhận diện đúng trên 85%
            ...
            def recognize(img,clf,faceCascade):#nhận diện khuôn mặt
                ...
                for face in result:
                    x, y, w, h = face['box']
                    face_img = img[y:y + h, x:x + w]  # 提取人臉區域

                    # 將提取的人臉區域轉換為灰度圖像
                    gray_face = cv2.cvtColor(face_img, cv2.COLOR_BGR2GRAY)

                    # 使用 LBPH 進行人臉辨識
                    label, confidence = clf.predict(gray_face)

                    # 在原圖上畫出人臉區域和辨識結果
                    cv2.rectangle(img, (x, y), (x + w, y + h), (255, 25, 255), 2)
                    cv2.putText(img, f"Label: {label}, Confidence: {confidence}", (x, y - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 25, 255), 2)

                    # 調用draw_boundray函數進行人臉識別
