...
class Attendance:
    ...
    def exportCsv(self):
        try:
            ...
            if len(mydata)<1:#nếu ko có dữ liệu trong bảng
                messagebox.showerror("No data","No data to export file",parent=self.root)#in thông báo
                return  False

            #tạo dataframe có các cột : mã điểm danh,Id,tên học sinh,....
            df = pd.DataFrame(mydata,
                              columns=['Check-in code', 'Student ID', 'Student name', 'Regular class', 'Check-in time', 'Check-out time', 'Day', 'lesson ID', 'Status'])
    ...
    def openImage(self):#hàm mở ảnh đã điểm danh
        if self.var_atten_id == "":
            messagebox.showerror("Error", "Please select an ID to view the photo", parent=self.root)
            ...
            try:
                ...
                if(len(data)!=0):#nếu có data
                    ...
                    for i in data:
                        ...
                        mydata.append(i)
                    messagebox.showinfo("Notification","Yes "+str(len(data))+" record that meet the conditions",parent=self.root)
                    conn.commit()
                else:
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
    ...
    def today_data(self):#hàm lấy dữ liệu trong ngày
        try:
            ...
            if (len(data) != 0):#nếu có dữ liệu
                ...
                messagebox.showinfo("Notification", "Yes " + str(len(data)) + " today's records",parent=self.root)
                conn.commit()
