...
class InsertData:
    def __init__(self,root):
        self.root=root
        self.root.title("Manage Information")#tiêu đề
    ...
    def insert_stu(self):
        try:
            ...
            df = pd.read_excel(fln)  # 讀取選擇的Excel文件



            for index, row in df.iterrows():  # 將Excel文件中的數據存入mydata變量
                mydata.append((row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8],
                               row[9], row[10], row[11]))
    ...
    def insert_less(self):
        try:
            ...
            my_cursor.executemany(sql_insert_query, mydata)
            conn.commit()
            messagebox.showinfo("Notification:","Successfully added the schedule list!!!")
            conn.close()
        except Exception as es:
            messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
    ...
    def delete_lesson(self):#hàm xóa tất cả buổi học
            try:
                delete=messagebox.askyesno("Delete class session","Do you want to delete this class session list?",parent=self.root)
                if delete>0:
                        conn = mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)
                        my_cursor = conn.cursor()
                ...
                conn.close()
                messagebox.showinfo("Delete","Successfully deleted the list of classes",parent=self.root)
            except Exception as es:
                messagebox.showerror("Error",f"Due To:{str(es)}",parent=self.root)
