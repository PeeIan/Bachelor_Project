...
class Report:
            ...
            try:
                ...
                if(len(data)!=0):
                    ...
                    for i in data:
                        self.LateTable.insert("",END,values=i)
                        mydata.append(i)
                    messagebox.showinfo("Notification","Yes"+str(len(data))+"records that meet the condition",parent=self.root)
                    conn.commit()
                else:
                    self.LateTable.delete(*self.LateTable.get_children())
    ...
    def exportCsv(self):
        try:
            ...
            df = pd.DataFrame(mydata,
                              columns=['Student ID', 'Student name', 'Class', 'Date', 'Lesson ID','Status'])
            ...
            messagebox.showinfo("Export data", "Your data has been exported to the Attendance_CSV folder")
        except Exception as es:
            messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
            ...
            try:
                ...
                if(len(data)!=0):
                    ...
                    for i in data:
                        self.NoAttTable.insert("",END,values=i)
                        mydataNot.append(i)
                    messagebox.showinfo("Notification","Yes "+str(len(data))+"records match the condition",parent=self.root)
                    conn.commit()
                else:
                    self.NoAttTable.delete(*self.NoAttTable.get_children())
    ...
    def exportUnpresetCsv(self):
        try:
            ...
            ts = calendar.timegm(current_GMT)
            if len(mydataNot)<1:
                messagebox.showerror("No data available","There is no data to export",parent=self.root)
                return  False
            df = pd.DataFrame(mydataNot,
                              columns=['Student ID', 'Student Name', 'Class', 'Date', 'Class session ID','Status'])
            ...
            try:
                ...
                if(len(data)!=0):
                    ...
                    for i in data:
                        self.NotInTable.insert("",END,values=i)
                        mydataNotInAtt.append(i)
                    messagebox.showinfo("Notification","Yes "+str(len(data))+" records that satisfy the conditions",parent=self.root)
                    conn.commit()
                else:
                    self.NotInTable.delete(*self.NotInTable.get_children())
    ...
    def exportNotinCsv(self):
        try:
            ...
            if len(mydataNotInAtt)<1:
                ...
                return  False
            df = pd.DataFrame(mydataNotInAtt,columns= ['Student ID', 'Student name','Class','Date','Classroom','Class session ID'])
